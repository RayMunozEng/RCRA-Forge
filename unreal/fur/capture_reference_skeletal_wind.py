...
out=root/'recovered/skeletal-wind-20260907';out.mkdir(exist_ok=True)
...
labels=['still','walk-only','walk-wind-a','walk-wind-b','wet-walk-wind-a','wet-walk-wind-b','wind-only','stopped']
def apply_stage(i):
 source.set_play_rate(1 if 1<=i<=5 else 0)
 fur.set_weather(1 if i in (4,5) else 0,.12 if 2<=i<=6 else 0)
  ...
  try:
   ...
   if state['pending']:
    ...
    if status=='complete':
     ...
     row.update(label=label,bone_follow_error_cm=error,bone_positions=pose,wetness=materials[0].get_scalar_parameter_value('Wetness'),wind=materials[0].get_scalar_parameter_value('WindStrength'))
     state['rows'].append(row)
     unreal.FurViewportProbe.capture(str(out/(label+'.png')))
     if i+1==len(labels):
      (out/'report.json').write_text(json.dumps(dict(captures=state['rows'],limits='Source-hidden tutorial skeleton with full production fur, live wind, wetness and TAA. Tests combined activity and settling; not per-vertex combined accuracy or retail animation parity.'),indent=2));stop();return
     apply_stage(i+1)
